...
import { IUniswapV2PairABI } from "./_helpers/iUniswapV2PairABI";
...
import { Hash, erc20Abi, maxUint256, parseEther, parseUnits } from "viem";
...
const UniswapV2: NextPage = () => {
  ...
  const [tokenAddressA, setTokenAddressA] = useState<string>(Object.keys(tokenAddresses)[0]);
  const [tokenAddressB, setTokenAddressB] = useState<string>(Object.keys(tokenAddresses)[1]);
      ...
      <div className="flex items-center flex-col flex-grow pt-10">
        ...
        <div className="flex flex-col justify-center items-center bg-base-300 w-full mt-8 px-8 pt-6 pb-12">
          <div className="flex flex-col bg-base-100 px-10 py-10 text-center items-center w-full md:w-2/4 rounded-3xl mt-10">
            ...
            <div className="pt-4">
              <div
                className={trade && !isSwapPossible ? "tooltip tooltip-info tooltip-bottom tooltip-open" : ""}
                {...(trade && !isSwapPossible
                  ? {
                      "data-tip": getDataTipMessage(isTokenEnough, isEthEnough, tokenA),
                    }
                  : {})}
              >
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (tokenA.address === WETH9[chainId].address) {
                      // Swap ETH for tokens
                      result = await writeUniswapV2Router02Async({
                ...
                <div className="flex flex-col pt-12">
                  <div>
                    <button
                      className="btn btn-primary text-lg px-12 mt-2"
                      onClick={async () => {
                        const result = await writeERC20Async({
                          abi: erc20Abi,
                          address: tokenAddressA,
